	...
	return {
		...
		deleterecord: function () {
			...
			function recordDelete(recordId, deleteUrl, dependency, dependencyMessage) {
				if (dependency) {
					...
					swal({
						title: "Esta seguro",
						text: "Al eliminar esto, también se eliminarán todos sus registros relacionados. ¿Todavía desea eliminar este registro?",
						type: "warning",
						showCancelButton: true,
						showLoaderOnConfirm: true,
						closeOnConfirm: false,
		...
		subscription: function () {
			...
			$('#addSubscription').click(function () {
				if (x < max_fields) {
					x++; // Increment counter to Agregar fields

					// Clone base field set and clean it
					$("#servicesContainer>.row:first-child").clone().appendTo("#servicesContainer"); // Agregar plan subscription
